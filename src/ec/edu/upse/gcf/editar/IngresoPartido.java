...
public class IngresoPartido {
	...
	private List<Equipojugador> ejugadoresEuno = new ArrayList<>();
	...
	@Command
	@NotifyChange({"lblCampeonato","lblEquipouno", "ejugadoresEuno"})
	public void buscarEquipouno() {	
		if(ejugadoresEuno != null ) {
			ejugadoresEuno.clear();
		}
		if(lblCampeonato.getValue() != null && lblEquipouno.getValue() != null ) {
			...
			if(!ej.isEmpty()) {
				for(Equipojugador equipojugador : ej) {
					ejugadoresEuno.add(equipojugador);					
				}
			}
		}
	}
	...
	@Command
	@NotifyChange({"jugadoresEuno"})
	public void agregarEquipouno() {
		Jugador jugadorU = new Jugador();
		jugadoresEuno.add(jugadorU);				
	}	
	
	@Command
	@NotifyChange({"jugadoresEuno"})
	public void quitar(@BindingParam("jugadorUno") Jugador jugadorU ) {		
		jugadoresEuno.remove(jugadorU);				
	}	
	
	/** BOTONES PARA SANCIONES DE JUGADOR*/
	@Command
	@NotifyChange({"jugadorSEuno"})
	public void agregarSEquipouno() {
		Jugador jugadorSU = new Jugador();
		jugadorSEuno.add(jugadorSU);				
	}	
	
	@Command
	@NotifyChange({"jugadorSEuno"})
	public void quitarSEuno(@BindingParam("jugadorSUno") Jugador jugadorSU ) {		
		jugadorSEuno.remove(jugadorSU);				
	}	
				...
				public void onEvent(Event event) throws Exception {
					if (event.getName().equals("onYes")) {
						...
						for(Detallepartido det : lblJuno) {
							det.setEquipojugador(lblJuno.get(0).getEquipojugador());
							det.setGolpt(lblJuno.get(0).getGolpt());
							det.setGolst(lblJuno.get(0).getGolst());
	...
	public List<Equipojugador> getEjugadoresEuno() {
		return ejugadoresEuno;
	}

	public void setEjugadoresEuno(List<Equipojugador> ejugadoresEuno) {
		this.ejugadoresEuno = ejugadoresEuno;
	}
}